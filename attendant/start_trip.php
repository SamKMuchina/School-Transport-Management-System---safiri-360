<?php
/**
 * start_trip.php (ATTENDANT copy)
 *
 * Trip execution page for the ATTENDANT role.
 *
 * Location  : attendant/start_trip.php
 ...
 *
 * NOTE ON ROLE: This exact page also exists as driver/start_trip.php,
 * a separate copy hardcoded for the DRIVER role. The two copies are
...
$back_dashboard = 'attendant_dashboard.php';

$error_message = '';

// Fetch attendant_id from user_id
$staff_result = mysqli_query($conn, "SELECT attendant_id FROM attendants WHERE user_id = $user_id");
$staff        = mysqli_fetch_assoc($staff_result);
if (!$staff) {
    die("Staff record not found.");
}
$staff_id = (int)$staff['attendant_id'];
...
// Fetch trip details - verify trip is assigned to this attendant
$trip_sql = "SELECT t.trip_id, t.status, t.trip_type, t.tracking_link, t.start_time, t.end_time,
             ...
             WHERE t.trip_id = $trip_id AND t.attendant_id = $staff_id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'start_trip') {
    ...
    if (!in_array($new_trip_type, array('morning', 'evening'))) {
        ...
        $start_sql = "UPDATE trips SET status = 'IN_PROGRESS', start_time = NOW(),
                       ...
                       WHERE trip_id = $trip_id AND attendant_id = $staff_id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'end_trip') {
    ...
    $end_sql = "UPDATE trips SET status = 'COMPLETED', end_time = NOW()
                 WHERE trip_id = $trip_id AND attendant_id = $staff_id AND status = 'IN_PROGRESS'";
