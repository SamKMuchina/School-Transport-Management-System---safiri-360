 ...
 * manage_drivers.php
 *
 * Transport Manager can register, view, edit and activate/deactivate drivers.
 *
 * Location  : transport_manager/manage_drivers.php
 * Includes  : ../includes/db.php
 * Stylesheet: ../assets/css/style.css (linked in manage_drivers_view.php)
 *
 * Access: TRANSPORT_MANAGER only
 *
 * Features:
 * - Register new drivers (auto-creates linked user account)
 * - Search drivers by name or phone (blank by default until search is done)
 * - Edit driver phone number (names are read-only)
 * - Activate/Deactivate driver accounts
 *
 * Business Rules:
 * - When adding a driver, a user account is created (password_hash = NULL)
 ...
 *   - Every numeric value (school_id, driver_id, user_id, new_status,
 ...
 *
 * FILE STRUCTURE: This file (manage_drivers.php) contains ALL PHP logic -
 * access control, form handling, and database queries. It stores the
 * results in plain variables, then includes manage_drivers_view.php,
...
// ============================================================
// SECTION 2: HANDLE ADD DRIVER
// ============================================================

/*
 * Registers a new driver and creates a user account.
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'add_driver') {
    ...
    if (empty($fname) || empty($lname) || empty($phone)) {
        ...
        try {
            // Check duplicate phone in drivers table for this school
            $check_sql    = "SELECT driver_id FROM drivers WHERE school_id = $school_id AND phone = '$phone_safe'";
            $check_result = mysqli_query($conn, $check_sql);
            if ($check_result && mysqli_num_rows($check_result) > 0) {
                throw new Exception('A driver with this phone number already exists.');
            }
            ...
            $insert_user_sql = "INSERT INTO users (username, password_hash, role, school_id, is_active, created_at)
                                 VALUES ('$new_username_safe', NULL, 'DRIVER', $school_id, 1, NOW())";
            mysqli_query($conn, $insert_user_sql);
            $new_user_id = $conn->insert_id;

            // Insert into drivers table
            $insert_driver_sql = "INSERT INTO drivers (school_id, fname, lname, phone, user_id)
                                   VALUES ($school_id, '$fname_safe', '$lname_safe', '$phone_safe', $new_user_id)";
            mysqli_query($conn, $insert_driver_sql);

            $conn->commit();
            $success_message = 'Driver registered successfully! A user account has been created.';
...
// ============================================================
// SECTION 3: HANDLE EDIT DRIVER (phone only)
// ============================================================

/*
 * Only the phone number can be updated.
 * Names are read-only because they are used for username generation.
 * Duplicate phone check excludes the current driver.
 */
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'edit_driver') {

    // (int) cast: driver_id is a number, so casting removes any chance
    ...
    if (empty($phone)) {
        ...
        // Check duplicate phone excluding this driver
        $check_sql    = "SELECT driver_id FROM drivers
                          WHERE school_id = $school_id AND phone = '$phone_safe' AND driver_id != $driver_id";
        $check_result = mysqli_query($conn, $check_sql);

        if ($check_result && mysqli_num_rows($check_result) > 0) {
            $error_message = 'A driver with this phone number already exists.';
        } else {
            $update_sql = "UPDATE drivers SET phone = '$phone_safe'
                            WHERE driver_id = $driver_id AND school_id = $school_id";
            if (mysqli_query($conn, $update_sql)) {
                $success_message = 'Driver phone updated successfully!';
            } else {
...
/*
 * Activates or deactivates a driver's user account.
 * Updates users.is_active using the user_id linked to the driver.
 */
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'toggle_status') {
    ...
    if (mysqli_query($conn, $update_sql)) {
        ...
        $success_message = "Driver " . $status_text . " successfully!";
    } else {
...
$count_sql     = "SELECT COUNT(*) as count FROM drivers WHERE school_id = $school_id";
$count_result  = mysqli_query($conn, $count_sql);
$row           = mysqli_fetch_assoc($count_result);
$total_drivers = $row['count'];

// ============================================================
// SECTION 6: SEARCH DRIVERS
// ============================================================

/*
 * Drivers are only shown when a search has been submitted.
 * By default ($search_query is empty) $drivers_result is null and the
...
if (!empty($search_query)) {
    ...
    $drivers_sql = "SELECT d.driver_id, d.fname, d.lname, d.phone, d.user_id, u.is_active
                     FROM drivers d
 ...
 * $error_message, $search_query, $total_drivers, and $drivers_result are
 * now ready to be displayed. manage_drivers_view.php contains no PHP
 * logic of its own - only HTML markup with these variables echoed/looped
 * into place.
 */
include 'manage_drivers_view.php';
